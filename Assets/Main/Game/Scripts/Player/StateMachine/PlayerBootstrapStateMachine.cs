    ...
    PlayerBootstrapStateMachine : MonoBehaviour
{
    ...
    private StateTransition[] GenerateStatesTransitions()
    {
        ...
        var enterPassiveStateTrans = new ByGameStateTransition(enterPassiveStateTransData, GameStateManager.Instance, enterCondition);
        var exitPassiveStateTransData = new BaseStateTransitionData(_states[1], null, false);
        var exitPassiveStateTrans = new FromPassiveStateTransition(exitPassiveStateTransData, _coreStateMachine, enterPassiveStateTrans, GameStateManager.Instance);
